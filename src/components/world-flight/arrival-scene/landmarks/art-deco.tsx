...
export function ArtDecoLandmark({ palette }: LandmarkLayerProps) {
  ...
  const Facade = ({ x, w, h }: { x: number; w: number; h: number }) => {
    const left = x - w / 2;
    return (
      <g>
        {/* body */}
        <rect x={left} y={-h} width={w} height={h} fill={f} />
        {/* central stepped parapet (ziggurat) + mast fin */}
        <rect x={x - 18} y={-h - 16} width={36} height={16} fill={f} />
        <rect x={x - 11} y={-h - 28} width={22} height={12} fill={f} />
        <rect x={x - 2.5} y={-h - 52} width={5} height={24} fill={f} />
  ...
  return (
    <g aria-hidden>
      <Facade x={-120} w={108} h={176} />
      <Facade x={0} w={120} h={202} />
      <Facade x={120} w={108} h={158} />
    </g>
  );
}
