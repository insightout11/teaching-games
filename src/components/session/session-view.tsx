...
import { getDestinationById, WORLD_FLIGHT_ORIGIN_ID } from '@/data/world-flight/destinations';
...
export function SessionView({ session, cls, students: serverStudents, existingScores }: SessionViewProps) {
  ...
  const wfArrivalTimeOfDay = useMemo(
    () => {
      ...
      const origin = getDestinationById(lesson.lessonPlanContent?.originId ?? WORLD_FLIGHT_ORIGIN_ID) ?? wfDestination;
