...
interface ScoreRow { session_id: string; is_correct: boolean | null; accuracy_status: string | null; counts_for_accuracy: boolean | null; counts_for_leaderboard: boolean | null; scoring_version: number | null; streak_count: number; points: number; }
...
export async function POST(request: NextRequest) {
  ...
  const [{ data: allScores }, { data: allRounds }, { data: allNotes }] = await Promise.all([
    supabase
      .from('scores')
      .select('session_id, is_correct, accuracy_status, counts_for_accuracy, counts_for_leaderboard, scoring_version, streak_count, points')
  ...
  const summaries = (sessions as SessionRow[]).map((session) => {
    ...
    const correct = scorable.filter(isCorrectScore).length;
